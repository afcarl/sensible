...
def get_track_specialist(tmpdir, bsm_port=6669, run_for=20.0, frequency=5):
    ...
    return TrackSpecialist(sensors, bsm_port, run_for, p, frequency=frequency, verbose=False)
...
def test_radar_track(tmpdir):
    ...
    synchronizer = Synchronizer(publish_freq=5, queue=radar_parser.queue, port=local_port,
                                topic=radar_parser.topic(), verbose=False, name="RadarSynchronizer")
    synchronizer.start()
    radar.start()

    track_specialist = get_track_specialist(tmpdir, run_for=30)
