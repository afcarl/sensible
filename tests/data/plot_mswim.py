...
for l in lights:
    ...
    hfont2 = {'fontname': 'Arial', 'size': 20}

    # add some text for labels, title and axes ticks
    ax.set_ylabel('RMSE (meters)', hfont2)
    ax.set_title('{}-{}-light'.format(run_name, light(l)), **hfont)
    ax.set_xticks(ind + width / 2)
    ax.set_yticks(**hfont2)
    ...
    fig.savefig(os.path.join('results', '{}-{}-light.png'.format(run_name, light(l))), dpi=100)
    plt.show()