...
def positionData(data, num):
    ...
    InterprettedArray = {'objMessage': 'Track', 'objID': objID, 'objLength': objLength, 'yVel': yposvel,
                         'xVel': xposvel, 'yPos': ypos, 'xPos': xpos, 'objZone': -1}
            ...
            if len(incoming_data) is 33:
                if incoming_data[22] == status_id_type:
                    ...
                    for var in incoming_data[-7:]:
                        binary_stuff += var
                    obtainedData = positionData(bin(int((binary_stuff), 16))[2:], incoming_data[-10])
                    cars.append(obtainedData)
                    # car_check
                    for box in range(0, 10):
                        if (((front_line_xs[box] + line_lengths[box]) >= float(obtainedData['xPos']) >= front_line_xs[
                            ...
                            float(obtainedData['yPos']) - (car_width / 2)) <= (left_line_ys[box] + trigger_offset))):
